...
static int handle_open(void *ctx, void *data, size_t data_sz)
{
	...
	printf("%-8s %-5s %-16d %-7d %-7d %-16d %-15s", ts, "OPEN ", e->timestamp_ns, e->conn_id.pid, e->conn_id.fd,
...
static int handle_close(void *ctx, void *data, size_t data_sz)
{
	...
	printf("%-8s %-5s %-16d %-7d %-7d %-16d %-9d %-9d", ts, "CLOSE", e->timestamp_ns, e->conn_id.pid, e->conn_id.fd,
...
static int handle_data(void *ctx, void *data, size_t data_sz)
{
	...
	printf("%-8s %-5s %-16d %-7d %-7d %-7d %-7s %-7d %-7d %-16s", ts, "DATA ", e->attr.timestamp_ns, e->attr.conn_id.pid, e->attr.conn_id.fd,
